...
def render() -> None:
    ...
    for label, value, detail in (
        ("Governance health", "92%", "Stable across active surfaces"),
        ("Open contributions", "07", "Two require review"),
        ("Release readiness", "Ready", "No blocking gate failures"),
    ):
    ...
    st.markdown(
        '<div class="traceboard-grid traceboard-grid--2" style="margin-top:16px">',
        unsafe_allow_html=True,
    )
    with st.container(border=False):
        panel(
            "Active work", "The decisions currently consuming review capacity.", elevation="raised"
        )
        table(
    ...
    with st.container(border=False):
        ...
        for source in SOURCES:
            st.markdown(
                f"- {source['title']} · {badge(source['status'], 'success')}",
                unsafe_allow_html=True,
            )
        close_panel()
    st.markdown("</div>", unsafe_allow_html=True)
